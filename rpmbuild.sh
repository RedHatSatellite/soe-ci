...
function build_srpm {
    ...
    then
        ...
        then
            ...
            /usr/bin/mock --offline --buildsrpm --spec ${SPECFILE} --sources $(pwd) --resultdir ${WORKSPACE}/tmp/srpms
            RETVAL=$?
            if [[ ${RETVAL} != 0 ]]
            then
            ...
            /usr/bin/mock --offline --rebuild ${srpmname} -D "%debug_package %{nil}" --resultdir ${YUM_REPO}
            RETVAL=$?
            if [[ ${RETVAL} != 0 ]]
            then
